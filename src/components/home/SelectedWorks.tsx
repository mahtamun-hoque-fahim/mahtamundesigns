...
export function SelectedWorks() {
  ...
  const title = c('home', 'selectedworks', 'title', 'Selected Works');
      ...
      <div className="container mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.6 }} className="mt-12 text-center">
          <Button variant="heroOutline" size="lg" asChild>
            <Link to="/clients">{c('home', 'selectedworks', 'button', 'View Portfolio')} <ArrowRight className="ml-2 w-4 h-4" /></Link>
          </Button>
        </motion.div>
      </div>
    </section>
