...
3. **Featured Projects** (`components/sections/featured-projects.tsx`) — "Showcase" eyebrow + "FEATURED PROJECTS" heading (renamed from "Portfolio"/"SELECTED WORKS"), asymmetric grid: 1 large featured card full-width on top (`md:col-span-2`), 2 smaller cards below side by side. "SEE ALL" button.
   ...
   - **Card heights** — increased 30% per Fahim's instruction: featured card `min-h-[481px]` (was `370px`), smaller cards `min-h-[377px]` (was `290px`). Note: values had already drifted from the original `280/220px` baseline by the time this request landed — always `grep` current values before doing percentage-based size changes rather than assuming the last-logged number is still accurate.
   - **Bottom glow color — fixed to match the actual background asset.** Was `rgba(187,124,255,0.35)` (the site's `--color-accent` token, lighter/pinker). Sampled actual pixel color from `featured-projects-bg.webp`'s glow region — peak was `rgb(160,83,242)`, a deeper violet. Updated to `rgba(160,83,242,0.4)` so the code-generated glow matches the image-baked glow exactly. Don't default to `--color-accent` for glows meant to match this specific background asset — sample the real pixel color instead.
