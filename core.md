...
5. **Reviews / Impressions** (`components/sections/reviews.tsx`) — light/white background (`bg-surface-light`, `text-ink`), contrast break from the rest of the dark homepage. "Customer Reviews and" eyebrow + "IMPRESSIONS" heading, "SEE ALL" button after.
   ...
   - **"SEE ALL" button — now a gradient, not solid.** Fahim clarified: the button uses a purple gradient, while the section's left-edge glow uses ONE of those same colors but as a solid (not gradient) fade-to-transparent. Implemented: `bg-gradient-to-r from-accent to-[#A053F2]` — `#A053F2` is the same `rgb(160,83,242)` sampled earlier from `featured-projects-bg.webp`, reused here so the button and the section glow share a color family instead of introducing a new unrelated purple.
