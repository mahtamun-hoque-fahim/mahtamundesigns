...
5. **Reviews / Impressions** (`components/sections/reviews.tsx`) — light/white background (`bg-surface-light`, `text-ink`), contrast break from the rest of the dark homepage. "Customer Reviews and" eyebrow + "IMPRESSIONS" heading, "SEE ALL" button after.
   ...
   - **"SEE ALL" button style — fixed to match reference.** Was an outline/bordered button (transparent bg, dark border) matching the light section's tone. Fahim's reference showed a solid purple pill button (`rounded-full bg-accent text-white`) with a purple glow (`shadow-[0_0_40px_rgba(187,124,255,0.55)]`, intensifying on hover). Updated to match exactly. **Open question, not yet resolved:** Fahim also sent a second reference image showing a review card floating against a dark purple-to-black gradient backdrop (not the flat white background this section currently uses) — unclear if this means the WHOLE section background needs to change, or if it's just showing the transition zone from Featured Projects' bottom glow bleeding into Reviews. Did not change the overall white background since the original full reference screenshot clearly showed white/light throughout this section — flagged here in case Fahim raises it again.
