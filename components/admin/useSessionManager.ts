...
// Lazy supabase getter for browser-only code
...
async function detectSuspiciousLogin(
  ...
  const { data: history } = await (getSupabase() as any)
    .from("login_history")
  ...
  const { data: recentFails } = await (getSupabase() as any)
    .from("login_history")
    .select("id")
    .eq("success", false)
...
async function saveSecurityAlerts(
  ...
  await (getSupabase() as any).from("security_alerts").insert(rows);
}
...
export async function recordLogin(userId: string, method: string = "email") {
  ...
  // Create new session
  await (getSupabase() as any).from("admin_sessions").insert({
    user_id: userId,
  ...
  await (getSupabase() as any).from("login_history").insert({
    user_id: userId,
    ...deviceInfo,
    success: true,
    login_method: method,
