...
2. **Hero** (`components/sections/hero.tsx`) — full-bleed background image (`public/images/hero-bg.webp`, pattern+glow baked into the asset by Fahim, NOT built as CSS/SVG), name headline (font-display, Space Grotesk), role subtitle, Portfolio/Contact buttons with lucide-react icons, portrait cutout (`public/images/hero-portrait.webp`, RGBA/transparent, confirmed alpha survives WebP conversion).
   ...
   - **Text block position** — vertical: moved DOWN (`translate-y-10`, `md:translate-y-16` — positive, not negative). Previous attempt moved it UP which was wrong direction; corrected per explicit instruction. Horizontal: pushed further right via container padding (`md:pl-20 lg:pl-28`, asymmetric — right side stays at `md:pr-10`) — Fahim asked for this multiple times, it wasn't a subtle nudge, it needed a real padding change, not a token-level tweak. If either axis is still off, ask for a specific direction/amount rather than guessing again.
   - **Three-dot decorative element** above the H1 — carried over from Fahim's original reference screenshot. Fahim questioned it (2026-07-03) but did not ask for removal — currently kept. If asked again, default to removing it since its purpose is unclear and it's been flagged twice.
