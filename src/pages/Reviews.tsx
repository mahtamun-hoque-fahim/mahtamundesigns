...
const Reviews = () => {
  ...
  const title = c('reviews', 'header', 'title', 'Client Reviews');
        ...
        <div className="container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-16">
            ...
            <p className="mt-4 text-lg text-muted-foreground max-w-xl">
              {c('reviews', 'header', 'description', "Hear from the brands we've helped transform through design.")}
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            {reviews.map((review, i) => (
              <motion.div key={review.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.06 }} className={cardSpans[i] || "md:col-span-4"}>
                <div className="bg-card border border-border/50 rounded-xl p-6 cursor-pointer hover:border-primary/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300" onClick={() => setExpanded(review.id)}>
