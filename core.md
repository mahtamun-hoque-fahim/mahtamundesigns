...
- **Shared horizontal baseline — HARD RULE (confirmed 2026-07-03):** every section's content must align to the exact same left/right margin: `max-w-[1400px]` container, `px-6 md:px-10` padding. Navbar, Hero, Featured Projects, and every future section must use this identical pattern — no section-specific extra padding, no asymmetric push. **Trusted-by strip is the ONLY exception** — it bleeds full-width edge to edge by design (the marquee needs to run off both edges). Two past mistakes that broke this and got reverted: Hero's `md:pl-20 lg:pl-28` push-right (from "push text more right"), and Featured Projects' `lg:px-12 xl:px-24` grid narrowing (from "minimize card width"). Both of those requests were real at the time, but this baseline rule now overrides them — if a future request would break the shared baseline again, flag the conflict to Fahim rather than silently applying it.
