...
3. **Featured Projects** (`components/sections/featured-projects.tsx`) — "Showcase" eyebrow + "FEATURED PROJECTS" heading (renamed from "Portfolio"/"SELECTED WORKS"), asymmetric grid: 1 large featured card full-width on top (`md:col-span-2`), 2 smaller cards below side by side. "SEE ALL" button.
   ...
   - **Background:** `public/images/featured-projects-bg.webp` (1920×2485, RGBA, self-fades to opaque black via alpha channel — top is semi-transparent tribal pattern, bottom is fully opaque black). Section background color is pure `bg-black` (NOT the `--color-bg` token, which is `#0a0a0a` — Fahim wants true pure black here specifically). Image is `width/height` sized (not `fill`), positioned `top-0`, `w-full h-auto object-contain object-top` so it sits at the top and the asset's own alpha fade handles the transition to black — no manual gradient overlay needed.
