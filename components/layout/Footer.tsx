...
import type { CmsMap, MediaMap } from '@/lib/supabase/queries'
import { getCms, getMedia } from '@/lib/supabase/queries'

export function Footer({ cms, mediaMap }: { cms: CmsMap; mediaMap: MediaMap }) {
          ...
          <div>
            ...
            <nav className="flex flex-col gap-3">
              {[
                ...
                <Link key={link.path} href={link.path} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                  {link.label}
                </Link>
              ))}
